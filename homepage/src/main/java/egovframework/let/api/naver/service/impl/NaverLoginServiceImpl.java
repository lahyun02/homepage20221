...
public class NaverLoginServiceImpl extends EgovAbstractServiceImpl implements NaverLoginService{
	...
	public String getAuthorizationUrl(HttpSession session, String domain) {
		String redirectUri = "http://" + domain + REDIRECT_URI;		// domain + ":8081" 포트번호 작성해야함.
		...
		//.apiKey() ~.state(state) : 앞서 생성한 난수값을 인증 URL 생성시 사용함
	...
	public String getUserProfile(OAuth2AccessToken oauthToken) throws IOException{
		...
		Request request = new Request.Builder().url(PROFILE_API_URL).method("GET", null).addHeader("Content-Type", "application/x-www-form-urlencoded").addHeader("Authorization", "Bearer " + oauthToken.getAccessToken()).build();
		//Bearer : api에서 기본으로 사용하는 문구
